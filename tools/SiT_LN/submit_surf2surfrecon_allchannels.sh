...
cd ${netmat2surf_path}
sbatch ./tools/SiT_LN/submit_top2conn_train_surf2surfrecon.sh # submits the first one which is chnl==0
...
do
    ...
    sed -i "s/\bspecific_channel: ${initial}\b/specific_channel: ${next}/g" "${yaml_loc}/hparams_SiTLN_surf2surfrecon.yml"
    sed -i "s/\bspecific_channel_end: ${initial}\b/specific_channel_end: ${next}/g" "${yaml_loc}/hparams_SiTLN_surf2surfrecon.yml"
    ...
    sbatch submit_top2conn_train_surf2surfrecon.sh
