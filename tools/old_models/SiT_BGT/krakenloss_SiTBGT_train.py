...
#             # dec_input = targets
#             pred = model(src=mesh_indata, tgt=targets,  tgt_mask=generate_subsequent_mask(model.latent_length).to(device))
...
def whole_model_arch(config):
    ...
    # fcn_train = getattr(functions_train, "train_SiTBGT_krakenonly_autoregressive")  #getattr(functions_train, config['training']['fcn_train'])  
    ...
    TEST_FLAG = config['logging']['sanity_file_pth']
    ...
    # tr_loader, val_loader, mean_train_label = fcn_prep_data_get_loaders(train_netmat=train_netmat_np, train_surface=train_surf_np, validation_netmat=val_netmat_np, validation_surface=val_surf_np, parcellation_N=from_parcellation, netmat_prep_choice=netmat_prep_choise, b_sz=batch_size, write_fpath=write_fpath)
    ...
    # dec_input_dim = int( upper_tri_sz + padding ) #config['transformer']['dec_input_dim']
    ...
    if TEST_FLAG:
        ...
        with torch.no_grad():
            ...
            #     pred = allvals[0]
            #     pred = pred[padding:] #these should have padding cause once its embeeded into dataloader, netmats do have padding.
            #     targets = targets[:, padding:]
