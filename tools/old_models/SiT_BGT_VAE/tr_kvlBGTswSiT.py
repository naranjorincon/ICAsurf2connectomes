...
def train_demean_BGTswMSSiT(model, krak_mse_weight, krak_latent_weight, krak_corrI_weight, train_loader, mean_train_label, device, optimizer, write_fpath, reset_params=True):
    ...
    for i, data in enumerate(train_loader):
        ...
        train_num_sub, num_chnl, num_verteces = pred.shape # first is batch size
        ...
        mae = (np.sum( np.abs(demean_targets.cpu().detach().numpy() -  demean_pred.cpu().detach().numpy()))) // train_num_sub
        train_mae_subs.append(mae)
        mse = (np.sum( (demean_targets.cpu().detach().numpy() - demean_pred.cpu().detach().numpy())**2)) // train_num_sub
    ...
    across_sub_mae_mean = np.sum(train_mae_subs) // len(train_mae_subs)# avg across batches
    across_sub_mse_mean = np.sum(train_mse_subs) // len(train_mse_subs)
    across_sub_corr_demean = np.sum(train_corr_demean_subs) // len(train_corr_demean_subs)
    across_sub_corr_orig = np.sum(train_corr_orig_subs) // len(train_corr_orig_subs)
...
def whole_model_arch(config):
    ...
    train_surf_np = np.load(f"{data_root_path}/surface-vision-transformers/data/ICAd15_schfd100/template/1L_train_data_ico6.npy") #data = sruf
    ...
    val_surf_np = np.load(f"{data_root_path}/surface-vision-transformers/data/ICAd15_schfd100/template/1L_validation_data_ico6.npy") #data = sruf
