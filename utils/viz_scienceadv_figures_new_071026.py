...
datasets = sys.argv[1] #"ABCD" #ABCD or infomap_prior_ABCDdr
print(f"dataset chosen is {datasets}")
model_type = "SiTLN" #"multimodalVAE" #"SiTLN"
topo_chosen = "ICA" if datasets == 'ABCD' else "infomap_prior_ABCDdr"
topo_dim = 15 if topo_chosen == 'ICA' else 20
version = "normICAdemeanNETMAT" if topo_chosen == 'ICA' else "normIMdemeanNETMAT"
model_test_type = "MSE"
val_step=1
img_extension = 'png' # png and made avriable in case we need to change it in the future to other stuff
out_of_distribution=False
partial_corr=bool(int(sys.argv[2]))
print(f"\nPartial corr condition is {partial_corr}\n")
# extract models and save each so that future images are for all models
if partial_corr is True:
    if datasets == "ABCD":
        from_parcellation_list = [100,15] # we only have aprtial for schaefer 100
        from_atlas_list = ["schf", "ICAnetmat"]
        netmat_id_version=["schaefer_mats", "ICAnetmat_mats"]
    elif datasets == "infomap_prior_ABCDdr":
        from_parcellation_list = [100,20] # we only have aprtial for schaefer 100
        from_atlas_list = ["schf", "INFOMAPnetmat"]
        netmat_id_version=["schaefer_mats", "INFOMAPnetmat_mats"]
else:
    if datasets == "ABCD":
        from_parcellation_list = [360,300,100,15]
        from_atlas_list = ["glasser", "schf", "schf", "ICAnetmat"]
        netmat_id_version=["glasser_mats", "schaefer_mats", "schaefer_mats", "ICAnetmat_mats"]
    elif datasets == "infomap_prior_ABCDdr":
        from_parcellation_list = [360,300,100,20]
        from_atlas_list = ["glasser", "schf", "schf", "INFOMAPnetmat"]
        netmat_id_version=["glasser_mats", "schaefer_mats", "schaefer_mats", "INFOMAPnetmat_mats"]

corr_type = "partial" if partial_corr is True else "full"

translation_models = []
for cc in range(len(from_parcellation_list)):
    from_parcellation = from_parcellation_list[cc]
    print(from_parcellation)
    from_atlas = from_atlas_list[cc]
    # translation=f"ICAd15_{from_atlas}d{from_parcellation}" # needs to be "" type of string
    translation=f"{topo_chosen}d{topo_dim}_{from_atlas}d{from_parcellation}"
    if datasets == "ABCD" and from_parcellation > 15: # over ride in this case
        ...
        model_details=f"120325_d6h3_tiny_adamW_cosinedecay_recon_MSEtrain_1L_{corr_type}_demean_exp{from_parcellation}_wGelu"
    else:
        model_type="SiTLN"
        version="normICAdemeanNETMAT"
        model_details = f"070426_d6h3_tiny_adamW_cosinedecay_1L_{corr_type}_{translation}_demean_wGelu_ico02"
    # if partial_corr is True:
    #     model_details = f"070426_d6h3_tiny_adamW_cosinedecay_1L_partial_{topo_chosen}d{topo_dim}_{from_atlas}d{from_parcellation}_demean_wGelu_ico02"

    print(model_details)
    print(f"{root}/model_out/{translation}/{datasets}/{model_type}/{version}/{model_details}")
    print()
...
print(f"{translation}, {model_details}, {from_atlas}, {from_parcellation}")
...
# %%
if partial_corr is True:
    directory = f'{root}/images/scienceadv_image/{datasets}/revisions/partial_{from_parcellation_list[cc]}' #+ '/ABCD_train_HCPYA_test'# Replace with your target directory
else:
    directory = f'{root}/images/scienceadv_image/{datasets}/revisions' #+ '/ABCD_train_HCPYA_test'# Replace with your target directory
...
# %%
iidx_model=cc
if datasets == "ABCD" and from_parcellation > 15:
    model_type="kSiTLN"
    version="normICAdemeanMAT"
else:
    model_type="SiTLN"
...
if "demean" in version: # if version is already demeaned, need to get actual MEAN from original data
    ...
    if "glasserd360" in translation:
        load_path=f"{scratch_path}/NeuroTranslate/brain_reps_datasets/{datasets}/maps_and_netmats/topo2glasserd360_{corr_type}"
    elif f"{topo_chosen}netmat" in translation:
        load_path=f"{scratch_path}/NeuroTranslate/brain_reps_datasets/{datasets}/maps_and_netmats/topo2toponetmat_{corr_type}"
    else:
        load_path=f"{scratch_path}/NeuroTranslate/brain_reps_datasets/{datasets}/maps_and_netmats/topo2schaeferd{from_parcellation}_{corr_type}"
    
    pathhh = f"{directory}/train_mean_flatten_d{from_parcellation}.npy"
    if not os.path.isfile(pathhh): #file does not exist
        print(f"File does not exist in: {pathhh}, so loading and calculating means.")
        train_netmat_np = np.load(f"{load_path}/train_{hemi_cond}_vecnetmat_uppertri.npy")
        ...
        if out_of_distribution:
            ...
            te_netmat_np = np.load(f"{load_path}/test_{hemi_cond}_vecnetmat_uppertri.npy")
...
# %%
if not datasets == "infomap_prior_ABCDdr":
    ...
    for cci in range(len(chnl_list)):
        ...
        directory_icarecon = f'{root}/images/{datasets}/ICAd15_ICAd15/SiTLN/{version_ica}/{model_details}/{model_test_type}'
    ...
    df_ica_individual = pd.DataFrame({
        ...
        "Groups": ["ICA 1"] * test_subject_length
        + ["ICA 2"] * test_subject_length + ["ICA 3"] * test_subject_length
        + ["ICA 4"] * test_subject_length + ["ICA 5"] * test_subject_length
        + ["ICA 6"] * test_subject_length + ["ICA 7"] * test_subject_length
        + ["ICA 8"] * test_subject_length + ["ICA 9"] * test_subject_length
        + ["ICA 10"] * test_subject_length + ["ICA 11"] * test_subject_length
        + ["ICA 12"] * test_subject_length + ["ICA 13"] * test_subject_length
        + ["ICA 14"] * test_subject_length + ["ICA 15"] * test_subject_length})
...
if datasets == "ABCD":
    ...
    directory100 = root + f'/images/{datasets}/{topo_chosen}d{topo_dim}_schfd100/kSiTLN/normICAdemeanMAT/' + model_details + '/' +model_test_type 
else:
    model_details=f"070426_d6h3_tiny_adamW_cosinedecay_1L_full_{topo_chosen}d{topo_dim}_schfd100_demean_wGelu_ico02"
    directory100 = root + f'/images/{datasets}/{topo_chosen}d{topo_dim}_schfd100/SiTLN/' + version + '/' + model_details + '/' +model_test_type 
...
if datasets == "ABCD" and from_parcellation > 15:
    #schf100_R
    model_details="070426_d6h3_tiny_adamW_cosinedecay_1R_full_ICAd15_schfd100_demean_1R_wGelu_ico02"
    directory100_R = root + f'/images/{datasets}/{topo_chosen}d{topo_dim}_schfd100/SiTLN/normICAdemeanNETMAT/' + model_details + '/' +model_test_type 
    translation_ica_schf100_R = pd.read_csv(directory100_R+'/allsubjects_individual_true_pred_testavg.csv')
    translation_ica_schf100_R = translation_ica_schf100_R[translation_ica_schf100_R['group'] == 'TEST_dmn'] #filter to get test_demean only
    print(f"schf100 average:{np.mean(translation_ica_schf100_R)} SD:{np.std(translation_ica_schf100_R)}")

    #schf100_bilateral
    model_details="070426_d6h3_tiny_adamW_cosinedecay_1L_full_ICAd15_schfd100_demean_bilateral_wGelu_ico02"
    directory100_BI = root + f'/images/{datasets}/{topo_chosen}d{topo_dim}_schfd100/SiTLN/normICAdemeanNETMAT/' + model_details + '/' +model_test_type 
    translation_ica_schf100_BI = pd.read_csv(directory100_BI+'/allsubjects_individual_true_pred_testavg.csv')
    translation_ica_schf100_BI = translation_ica_schf100_BI[translation_ica_schf100_BI['group'] == 'TEST_dmn'] #filter to get test_demean only
    print(f"schf100 average:{np.mean(translation_ica_schf100_BI)} SD:{np.std(translation_ica_schf100_BI)}")

#schf100_partial
if datasets == "ABCD":
    model_details="120325_d6h3_tiny_adamW_cosinedecay_recon_MSEtrain_1L_Partial_demean_exp100_wGelu"
    directory100_partial = root + f'/images/{datasets}/{topo_chosen}d{topo_dim}_schfd100/kSiTLN/normICAdemeanMAT/'+ model_details + '/' +model_test_type 
else:
    model_details=f"070426_d6h3_tiny_adamW_cosinedecay_1L_partial_{topo_chosen}d{topo_dim}_schfd100_demean_wGelu_ico02"
    directory100_partial = root + f'/images/{datasets}/{topo_chosen}d{topo_dim}_schfd100/SiTLN/' + version + '/' + model_details + '/' +model_test_type 
...
if datasets == "ABCD":
    ...
    directory300 = f'{root}/images/{datasets}/{topo_chosen}d{topo_dim}_schfd300/kSiTLN/normICAdemeanMAT/'+ model_details + '/' +model_test_type 
else:
    model_details=f"070426_d6h3_tiny_adamW_cosinedecay_1L_full_{topo_chosen}d{topo_dim}_schfd300_demean_wGelu_ico02"
    directory300 = root + f'/images/{datasets}/{topo_chosen}d{topo_dim}_schfd300/SiTLN/' + version + '/' + model_details + '/' +model_test_type 
...
if datasets == "ABCD":
    ...
    directory360 = root + f'/images/{datasets}/{topo_chosen}d{topo_dim}_glasserd360/kSiTLN/normICAdemeanMAT/' + model_details + '/' +model_test_type 
else:
    model_details=f"070426_d6h3_tiny_adamW_cosinedecay_1L_full_{topo_chosen}d{topo_dim}_glasserd360_demean_wGelu_ico02"
    directory360 = root + f'/images/{datasets}/{topo_chosen}d{topo_dim}_glasserd360/SiTLN/' + version + '/' + model_details + '/' +model_test_type 
...
# 15x15 full corr ICA netmats
model_details=f"070426_d6h3_tiny_adamW_cosinedecay_1L_full_{topo_chosen}d{topo_dim}_{topo_chosen}netmatd{topo_dim}_demean_wGelu_ico02"
directory15_full = root + f'/images/{datasets}/{topo_chosen}d{topo_dim}_{topo_chosen}netmatd{topo_dim}/SiTLN/normICAdemeanNETMAT/'+ model_details + '/' +model_test_type 
translation_ica_ica_full = pd.read_csv(f'{directory15_full}/allsubjects_individual_true_pred_testavg.csv')
translation_ica_ica_full = translation_ica_ica_full[translation_ica_ica_full['group'] == 'TEST_dmn'] #filter to get test_demean only
print(f"{topo_chosen} netmats full:{np.mean(translation_ica_ica_full)} SD:{np.std(translation_ica_ica_full)}")

# 15x15 partial corr ICA netmats
model_details=f"070426_d6h3_tiny_adamW_cosinedecay_1L_partial_{topo_chosen}d{topo_dim}_{topo_chosen}netmatd{topo_dim}_demean_wGelu_ico02"
directory15_partial = root + f'/images/{datasets}/{topo_chosen}d{topo_dim}_{topo_chosen}netmatd{topo_dim}/SiTLN/normICAdemeanNETMAT/'+model_details + '/' +model_test_type 
translation_ica_ica_partial = pd.read_csv(f'{directory15_partial}/allsubjects_individual_true_pred_testavg.csv')
translation_ica_ica_partial = translation_ica_ica_partial[translation_ica_ica_partial['group'] == 'TEST_dmn'] #filter to get test_demean only
print(f"{topo_chosen} netmats partial:{np.mean(translation_ica_ica_partial)} SD:{np.std(translation_ica_ica_partial)}")

df_list = []
df_all = pd.DataFrame({
    "Values": np.concatenate([translation_ica_schf100["value"],
        translation_ica_schf100_partial["value"],
        translation_ica_schf300["value"],
        translation_ica_glass360["value"],
        translation_ica_ica_full["value"],
        translation_ica_ica_partial["value"]]),
    ...
    + [f"{from_atlas}\n{topo_dim}"] * len(translation_ica_ica_full)
    + [f"{from_atlas}\n{topo_dim} PC"] * len(translation_ica_ica_partial)
})
...
if datasets == "ABCD" and from_parcellation > 15:
    df_all_11 = pd.DataFrame({
        "Values": np.concatenate([translation_ica_schf100["value"],
                                translation_ica_schf100_R["value"],
                                translation_ica_schf100_BI["value"]]),
        "Groups": ["Schaefer\n100 LEFT"] * len(translation_ica_schf100)
        + ["Schaefer\n100 RIGHT"] * len(translation_ica_schf100_R)
        + ["Schaefer\n100 BOTH"] * len(translation_ica_schf100_BI) 
    })

# %%
from matplotlib.colors import to_rgba
#scale=width to make these violin plots match. Spread of ICAnetmats are very large and mess up the violin plotting:
# https://stackoverflow.com/questions/56350531/seaborn-violinplot-experiences-difficulty-with-too-many-variables#
df_list_palette=[]
df_list_palette.append(['gray','lightsalmon','red','cyan', 'darkgreen', 'palegreen'])
if len(df_list)==1:
    fig = plt.figure(figsize=(20, 10))
    for iii in range(len(df_list)):
        axxx=sns.violinplot(df_list[iii], x="Groups", y="Values", palette=df_list_palette[iii], linewidth=5, scale='width')
        for violin, alpha in zip(axxx.collections[::2], [0, 0, 0, 0, 0,0]):
...
# axxx.set_yticks([-0.2, 0, 0.2, 0.4, 0.6, 0.8])
axxx.set_ylabel("Translation Acc.")
axxx.set_xlabel("")

filename = f"/violin_plot_avgRecon_translations_100_300_360.{img_extension}"
print(f"Saving to path:{directory}")
plt.tight_layout()
# Save the figure
if not local_flag:
...
if datasets == "ABCD":
    fig = plt.figure(figsize=(20, 10))
    axxx=sns.violinplot(df_all_11, x="Groups", y="Values", palette=df_list_palette[iii], linewidth=5)
    ...
    axxx.set_ylabel("Translation Acc.")
    axxx.set_xlabel("")
    filename = f"/translation_LEFT_RIGHT_BILATERAL.{img_extension}"
    print(f"Saving to path:{directory}")
    plt.tight_layout()
    # Save the figure
    if not local_flag:
...
if using_ica_mean_comparison:
    print(f"Checking with {topo_chosen} means")
...
print(f"slope: {txt}")
plt.tight_layout()
filename = f"/comparing_before_after_avg_added.{img_extension}"
print(f"Saving to path:{directory}")
plt.tight_layout()
# Save the figure
if not local_flag:
...
filename = f"/Pop_mean_stuff.{img_extension}"
print(f"Saving to path:{directory}")
plt.tight_layout()
# Save the figure
if not local_flag:
...
for pp in range(len(behv_of_interest_list)):
    ...
    if partial_corr is True:
        parcellation_corr_type="partial"
    else:
        parcellation_corr_type="full"

    parcellation_name = netmat_id_version[cc].split('_')[0]

    if translation == f"{topo_chosen}d{topo_dim}_{topo_chosen}netmatd{topo_dim}":
        matrix_type = "topography_maps"
    else:
        matrix_type = "connectome"
    ...
    ABCD_main_ids_clean=(get_subIDs_groups[get_subIDs_groups['group'] == 'test']['sub_IDs'].unique())
    print(f"Loaded in test IDs to do behavior stuff: {len(ABCD_main_ids_clean)}, {ABCD_main_ids_clean[:2]}")
    ...
    print(f"behv clean {len(unique_ids_clean)} brain data mask{len(isin_check_mask)}->{isin_check_mask.sum()} because turns into boolean mask for later. If you sum it, then that is number of matches between imaging sample and behavior sample. sum={isin_check_mask.sum()}")
    ...
    print(ABCD_main_ids_clean[:-2])
    print("Above and below should be equal?")
    print(unique_ids_clean_unified[:-2])
    ...
    print(len(subj_list_scores),subj_list_scores[:-2])
    ...
    print(ABCD_original_ABCD_behv_match_df_clean.shape)
...
table_of_test_performance_15mat_full = pd.read_csv(directory15_full+'/FingerPrintTable_avgrank.csv')["FingerPrintPerformance_avgrank"][:test_n]
table_of_test_performance_15mat_pc = pd.read_csv(directory15_partial+'/FingerPrintTable_avgrank.csv')["FingerPrintPerformance_avgrank"][:test_n]
...
print(np.mean(table_of_test_performance_15mat_full))
print(np.mean(table_of_test_performance_15mat_pc))

plt.figure(figsize=(20, 10))
    # axes = axes.flatten()
df = pd.DataFrame({
    "Values": np.concatenate([np.asarray(table_of_test_performance_100),
    np.asarray(table_of_test_performance_100_partial), 
    np.asarray(table_of_test_performance_300), 
    np.asarray(table_of_test_performance_360),
    np.asarray(table_of_test_performance_15mat_full),
    np.asarray(table_of_test_performance_15mat_pc)]),
    "Groups": ["Schaefer 100 AvgRank"] * len(table_of_test_performance_100) 
    + ["Schaefer 100 PC AvgRank"] * len(table_of_test_performance_100) 
    + ["Schaefer 300 AvgRank"] * len(table_of_test_performance_300) 
    + ["Schaefer 360 AvgRank"] * len(table_of_test_performance_360)
    + [f"{from_parcellation} {topo_dim} AvgRank"] * len(table_of_test_performance_15mat_full)
    + [f"{from_parcellation} {topo_dim} PC AvgRank"] * len(table_of_test_performance_15mat_pc)
})
# df = pd.DataFrame({
#     "Values": np.asarray(table_of_test_performance_100),
#     "Groups": ["Schaefer 100 AvgRank"] * len(table_of_test_performance_100)
# })
axxx=sns.histplot(df,x="Values", hue="Groups", bins=200, log_scale=[False, True], palette=['gray', 'lightsalmon', 'red','darkcyan', 'darkgreen', 'lightgreen'])
# sns.move_legend(axxx, "lower center", ncol=2, title=None, frameon=False, bbox_to_anchor=(0.5,1))
