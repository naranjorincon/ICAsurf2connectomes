...
def whole_model_arch(config):
    ...
    folder_to_save_model_test=f'{model_out_root}/{translation}/{dataset_choice}/{model_type}/{version}/{model_details}/{chosen_test_model}/ABCD_train_HCPYA_test'
    ...
    write_fpath = f"{data_root_path}/NeuroTranslate/surf2netmat/batch/{model_type}_{sanity_check_version}_{dataset_choice}_outofsampletest.print"

    # make necessary folders
    if not os.path.exists(folder_to_save_model_test):
        # Create the directory
        os.makedirs(folder_to_save_model_test)
        write_to_file(f'Creating dir: {folder_to_save_model_test}', filepath=write_fpath)
    ...
    if dataset_choice == "HCPYA":
        ...
        if parcellation_corr_type == "full":
            ...
            te_netmat_np = np.load(f"{data_root_path}/NeuroTranslate/brain_reps_datasets/{outofsample_dataset_choice}/schaefer_mats/netmat_d100/train_netmat_clean.npy")
            te_surf_np = np.load(f"{data_root_path}/NeuroTranslate/brain_reps_datasets/{outofsample_dataset_choice}/ICA_maps/ICAd15_ico02/{hemi_cond}_train_surf.npy")
            write_to_file(f'Loaded in TEST. They have shapes: {te_netmat_np.shape} & {te_surf_np.shape} respectively.', filepath=write_fpath)
    ...
    with torch.no_grad():
        ...
        np.save(f"{folder_to_save_model_test}/te_big_corr_matrix.npy", across_sub_rho) # save for viz later
        ...
        np.save(f"{folder_to_save_model_test}/te_max_netmat_translation.npy", te_max_netmat_translation)
        np.save(f"{folder_to_save_model_test}/te_min_netmat_translation.npy", te_min_netmat_translation)
        ...
        np.save(f"{folder_to_save_model_test}/tr_big_corr_matrix.npy", across_sub_rho) # save for viz later
        ...
        np.save(f"{folder_to_save_model_test}/tr_max_netmat_translation.npy", tr_max_netmat_translation)
        np.save(f"{folder_to_save_model_test}/tr_min_netmat_translation.npy", tr_min_netmat_translation)
    
    # save training losses
    df_version_mae = pd.DataFrame(mae_train_list)
    df_version_mae.to_csv(os.path.join(folder_to_save_model_test, 'mae_train_model.csv'))
    df_version_mse = pd.DataFrame(mse_train_list)
    df_version_mse.to_csv(os.path.join(folder_to_save_model_test, 'mse_train_model.csv'))

    # save test losses
    df_version_mae = pd.DataFrame(mae_test_list)
    df_version_mae.to_csv(os.path.join(folder_to_save_model_test, 'mae_test_model.csv'))
    df_version_mse = pd.DataFrame(mse_test_list)
    df_version_mse.to_csv(os.path.join(folder_to_save_model_test, 'mse_test_model.csv'))
    ...
    np.save(f"{folder_to_save_model_test}/train_ground_truth.npy", tr_ground_truth)
    np.save(f"{folder_to_save_model_test}/train_pred.npy", tr_pred)
    np.save(f"{folder_to_save_model_test}/test_ground_truth.npy", te_ground_truth)
    np.save(f"{folder_to_save_model_test}/test_pred.npy", te_pred)
