...
'''
This file implements the training procedure to train a MS-SiT model for segmentation.
'''
...
from tools.utils import logging_ms_sit
...
def train(config):
    ...
    #mesh_resolution
    ico_mesh = config['mesh_resolution']['ico_mesh'] #ico6 for patches
    ico_grid = config['mesh_resolution']['ico_grid'] #ico5
    num_patches = config['sub_ico_{}'.format(ico_grid)]['num_patches'] #20480
    num_vertices = config['sub_ico_{}'.format(ico_grid)]['num_vertices'] #6

    #data
    dataset = config['data']['dataset'] #HCP_YA
    task = config['data']['task'] #schfd100_ICAd15
    ...
    if config['MODEL'] == 'ms-sit':
        if config['transformer']['shifted_attention']:
            ...
            model = MSSiTUNet_shifted(ico_init_resolution=config['mesh_resolution']['ico_grid'],
                            ...
                            qk_scale=config['transformer']['qk_scale'],
    ...
    if  segmentation_task: 
        if loss == 'ce':
            ...
            print('Using {} and {}'.format(criterion1,criterion2))
    else: #SDNR Added condition because likely not going to use the dice loss
        if loss == 'mse':
        ...
        if (epoch+1)%val_epoch==0:
            ...
            with torch.no_grad():
                ...
                for i, data in enumerate(val_loader):
                    ...
                    preds_.append(torch.argmax(torch.nn.functional.softmax(outputs,dim=1),dim=1).detach().cpu().numpy()) # max probability
